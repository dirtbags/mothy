...
void showHello()
{
  display.setRotation(3); //even = portrait, odd = landscape
  display.fillScreen(GxEPD_WHITE);
  display.drawBitmap(hello, 30, 0, 296, 128, GxEPD_WHITE);
  display.drawBitmap(netarch, 0, 0, 67, 128, GxEPD_WHITE);
  const GFXfont* f = &FreeSansBoldOblique24pt7b;
  display.setTextColor(GxEPD_BLACK);
  //display.setTextColor(GxEPD_RED);  
  display.setFont(f);
  display.setCursor(90+30,100);
